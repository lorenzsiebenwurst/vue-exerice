Vue.createApp({
  ...
  computed: {
    tableHead() {
      ...
      const objectKeys = Object.keys(this.books[0]);
      return objectKeys.filter((key) => validKeys.includes(key));
    },

    filteredByAuthor() {
      demandedPublisher = "O'Reilly Media";
      return this.books.filter((book) => book.publisher === demandedPublisher);
    },
  },

  methods: {
    reducedPrice(price, percentage) {
      const [, priceOnly] = price.split("$");
      const discount = (percentage / 100) * priceOnly;
      return "$" + (priceOnly - discount).toFixed(2);
    },
  },
}).mount("#app");
